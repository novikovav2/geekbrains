...
function handleClick(clickedButtonEvent) {
    ...
    if (clickedButtonEvent.target.textContent === 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (clickedButtonEvent.target.textContent === 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let sometext = 'Lorem ipsum dolor sit amet, consectetur adipisicing elit.'
